...
MTK_BUILD_MANIFOLD(state_ikfom,//24维的状态量x
((vect3, pos))                         // pos：位置
((SO3, rot))                           // rot：欧拉角
((SO3, offset_R_L_I))         		   // offset_R_L_I：IMU和激光雷达之间的安装角（旋转矩阵）
((vect3, offset_T_L_I))      		   // offset_T_L_I：IMU和激光雷达之间的杆臂
((vect3, vel))                         // vel：速度
((vect3, bg))                          // bg：陀螺仪零偏
((vect3, ba))                          // ba：加速度计零偏
((S2, grav))                           // grav：重力向量
...
//噪声协方差Q的初始化(对应公式(8)的Q, 在IMU_Processing.hpp中使用)fast-lio 1
...
// fast_lio2论文公式(2), 这里的f就是将imu的积分方程组成矩阵形式然后再去计算
Eigen::Matrix<double, 24, 1> get_f(state_ikfom &s, const input_ikfom &in)
{// 对应顺序为速度(3)，角速度(3),外参T(3),外参旋转R(3)，加速度(3),角速度偏置(3),加速度偏置(3),位置（重力向量）(3)，与论文公式顺序不一致
...
// 对应fast_lio1/2论文公式(7) 注意该矩阵没乘dt，没加单位阵
Eigen::Matrix<double, 24, 23> df_dx(state_ikfom &s, const input_ikfom &in)
{//为pos(3), rot(3),offset_R_L_I(3),offset_T_L_I(3), vel(3), bg(3), ba(3), grav(2);这一块和fast-lio不同需要注意
	Eigen::Matrix<double, 24, 23> cov = Eigen::Matrix<double, 24, 23>::Zero();
	cov.template block<3, 3>(0, 12) = Eigen::Matrix3d::Identity();//R3的单位阵，代表速度转移//对应公式(7)第2行第3列   I
	vect3 acc_;
	in.acc.boxminus(acc_, s.ba);//加速度
	vect3 omega;
	in.gyro.boxminus(omega, s.bg);//角速度
	cov.template block<3, 3>(12, 3) = -s.rot.toRotationMatrix()*MTK::hat(acc_);//这里的-s.rot.toRotationMatrix()//对应公式(7)第3行第1列
	cov.template block<3, 3>(12, 18) = -s.rot.toRotationMatrix();// 将角度转到存入的矩阵中//对应公式(7)第3行第5列 
...
// 对应fast_lio1/2论文公式(7)注意该矩阵没乘dt
Eigen::Matrix<double, 24, 12> df_dw(state_ikfom &s, const input_ikfom &in)
{
	Eigen::Matrix<double, 24, 12> cov = Eigen::Matrix<double, 24, 12>::Zero();
	cov.template block<3, 3>(12, 3) = -s.rot.toRotationMatrix();//对应公式(7)第3行第2列  -R 
	cov.template block<3, 3>(3, 0) = -Eigen::Matrix3d::Identity();//对应公式(7)第1行第1列  -A(w dt)简化为-I
